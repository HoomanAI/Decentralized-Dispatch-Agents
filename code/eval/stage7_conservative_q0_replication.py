"""Extend the conservative q=0 comparison to the fixed total of 60 paired seeds."""
...
def main() -> None:
    ...
    partial = results / "stage7_conservative_q0_replication.partial.csv"
    output = results / "stage7_conservative_q0_replication.csv"
    frames = [pd.read_csv(partial)] if partial.exists() else []
    complete = set(frames[0].method.unique()) if frames else set()
    arms = [
    ...
    print(result.groupby("method").survival_total.mean())
